...
function Submit() {
  ...
  const SubmitData = async (e) => {
    e.preventDefault();
    setbuttonPopup(true);
    const { name, rollno, event, link } = user;
    const response = await fetch("https://kstar-api.herokuapp.com/api/submit", {
      method: "POST",
      headers: {
    ...
    setUser({  ...user, name: "", rollno: "", event: "", link: "" });
    if (body === "Link Updated") {
      setError("Submission Successful");
      setanimation(1);
    } else {
      setError("Please provide Registered Name, Rollno and Event");
      setanimation(2);
    }
  };  
  ...
  return (
    <div className="bg-gradient-to-bl from-purple via-blue to-blue w-full px-4 md:px-10 py-6">
      ...
      <div className="mx-auto flex lg:flex-nowrap flex-wrap items-center">
        {moment().format("YYYY-MM-DD HH:mm:ss") >= moment("2021-12-24 23:59:00").format("YYYY-MM-DD HH:mm:ss") &&
        moment().format("YYYY-MM-DD HH:mm:ss") < moment("2021-12-27 23:59:59").format("YYYY-MM-DD HH:mm:ss") ? (
          <div className="lg:w-1/2 flex flex-col w-full lg:px-12 md:mt-0 sm:mr-10">
            <img
              src="./assets/Registration/submission.png"
              alt="Registration"
            ...
            <div className="relative mb-4">
              <p className="font-bold text-viodeep text-center">
                Please provide the drive link of your submission. <br />
                Kindly change the accessibility of the drive link to all.
              </p>
            </div>
            ...
            <div className="w-full flex flex-wrap justify-center mt-6">
              ...
              user.link === "" ? (
                <p className="text-xs text-white font-bold mt-3 w-full text-center">
                  <span className="text-red-600 font-bold">* </span> Some fields
                  are mising
                </p>
              ) : (
                <p></p>
      ...
      <Popup trigger={buttonPopup} setTrigger={setbuttonPopup} animation={animation}>
